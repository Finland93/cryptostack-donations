 ...
 *   protected by an integrity hash (CSDON_TREASURY_FINGERPRINT). On every
 ...
 * the plugin's settings screen, and the donor sees every receiving address in
 * their own wallet before signing.
 */
if ( ! defined( 'CSDON_PLATFORM_FEE_BPS' ) ) {
	define( 'CSDON_PLATFORM_FEE_BPS', 100 ); // 1%.
}
 ...
 */
if ( ! defined( 'CSDON_TREASURY_EVM' ) ) {
	define( 'CSDON_TREASURY_EVM', '0x15a308ed14B7c519a474d1856f45bF81d0000D80' );
}
if ( ! defined( 'CSDON_TREASURY_SOL' ) ) {
	define( 'CSDON_TREASURY_SOL', 'GikaTNTG1DpytsTqwRdcVJ3F7Z2eHHgFZTkkUWxP2VRP' );
}
if ( ! defined( 'CSDON_TREASURY_BTC' ) ) {
	// Native SegWit (bech32).
	define( 'CSDON_TREASURY_BTC', 'bc1q2sg0hf2a6vqxsypwt28rwwt625ct3fq5z3na64' );
}
 ...
 * treasury address, regenerate this value with csdon_compute_treasury_fingerprint().
 */
if ( ! defined( 'CSDON_TREASURY_FINGERPRINT' ) ) {
	define(
		'CSDON_TREASURY_FINGERPRINT',
		hash(
			'sha256',
			CSDON_TREASURY_EVM . '|' . CSDON_TREASURY_SOL . '|' . CSDON_TREASURY_BTC . '|' . CSDON_PLATFORM_FEE_BPS
		)
	);
}
 ...
 */
function csdon_compute_treasury_fingerprint() {
	return hash(
		'sha256',
		CSDON_TREASURY_EVM . '|' . CSDON_TREASURY_SOL . '|' . CSDON_TREASURY_BTC . '|' . CSDON_PLATFORM_FEE_BPS
	);
 ...
 * @return bool
 */
function csdon_treasury_is_intact() {
	return hash_equals( CSDON_TREASURY_FINGERPRINT, csdon_compute_treasury_fingerprint() );
}
 ...
 * @return string|null
 */
function csdon_treasury_for_family( $family ) {
	switch ( $family ) {
		case 'evm':
			return CSDON_TREASURY_EVM;
		case 'solana':
			return CSDON_TREASURY_SOL;
		case 'bitcoin':
			return CSDON_TREASURY_BTC;
		default:
			return null;
	}
}
